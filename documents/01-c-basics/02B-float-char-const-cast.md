...
description: "掌握 C 语言的浮点类型与精度问题、字符存储与编码、const 限定符和隐式类型转换规则，踩清每一个会咬人的坑。"
chapter: 1
order: 3
tags:
  - host
...
说实话，这篇里有些东西——特别是隐式类型转换——初看会觉得很绕。但别担心，这些"坑"恰恰是 C 类型系统最该用心理解的地方。理解了"什么容易出问题"，后面写底层代码、调奇怪的数值 bug 就会顺很多。
...
下面这个例子用 `double` 来演示，因为双精度才能把"近似值"的尾巴露出来：
...
这里还有个细节值得亲手验证一下：`float` 只有约 7 位有效数字，精度比 `double` 低很多，有些在 `double` 下"露馅"的误差，截断到 `float` 后反而恰好对上了。所以同样的 `0.1 + 0.2 == 0.3` 用 `float`（带 `f` 后缀）去比，在本平台上**反而会相等**——但这只是精度太低把误差"抹平"了，并不代表浮点能精确存这些值。规律仍然成立：比浮点永远用 epsilon。
...
C 里有个反直觉的细节：字符字面量 `'A'` 的类型不是 `char`，而是 `int`。

```c
printf("%zu\n", sizeof('A'));  // C 中输出 4
```

原因是字符字面量要先经历**整型提升**（后面会讲），它在表达式里以 `int` 身份参与运算。这个细节在绝大多数情况下不影响你写代码，但 sizeof 的结果可能会让你愣一下——记住 `'A'` 在 C 里是个 `int` 就行。
...
C 风格 cast 的问题在于太"万能"了——`const` 可以被 cast 掉、指针类型可以随意转换、数据布局的假设完全没有验证。所以在 C 里用 cast 时，务必清楚自己每一步在做什么，最好在注释里写明意图，因为编译器不会再帮你兜底。
