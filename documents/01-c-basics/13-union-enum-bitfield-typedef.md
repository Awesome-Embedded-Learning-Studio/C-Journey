...
- 99
- 11
description: 掌握联合体、枚举、位域与 typedef 的使用,理解类型双关、硬件寄存器映射等技巧
difficulty: beginner
order: 13
platform: host
prerequisites:
...
这在 C 中是完全合法的。背后的关键在于 C99 标准里关于联合体的那条"附注"(footnote)——允许读取的是最近写入的那个成员之外的成员,用以实现类型双关。这也是为什么你在无数 C 工程里能看到这种写法,而它不会出问题。不过要注意保持位宽和表示方式一致(比如用 `uint32_t` 去对应 `float`,而不是随便拿个别的类型去读),否则读出来的就是垃圾。
...
int x = COLOR_RED;     // 合法!隐式转为 int
```

这种宽松在 C 语言看来是"灵活性",但从类型安全的角度看就是个口子——编译器完全没办法帮你检查"这个值是不是合法的枚举值"。所以实践中常见两种补救:一是给枚举加一个 `*_COUNT` 计数哨兵用于范围检查,二是写专门的校验函数,把合法区间挡死在运行期。
...
位域用起来很爽,但它有一个你必须正视的代价:**可移植性差**。C 标准对位域有几个关键细节没有规定——位域的分配顺序(从低位向高位还是反过来)、有符号位域的符号扩展、存储单元边界对齐,这些全部交给编译器实现。
...
typedef 创建的是一个**真正的类型别名**,由编译器处理;而 `#define` 只是预处理器的文本替换。这两者最经典的区别就出在声明多个变量时:
...
为什么 `d` 会变成 `char` 而不是 `char*`?因为宏只是把 `CHAR_PTR` 替换成 `char*`,替换完源代码实际上是 `char* c, d;`,而 C 的声明规则里 `*` 是跟着变量走的、不是跟着类型的,所以 `*` 只绑定了 `c`,`d` 就只剩下一个光秃秃的 `char`。typedef 不一样,它把 `char*` 整体绑定成一个新类型,`a` 和 `b` 自然都是这个新类型。
...
这一篇我们一口气讲了四个 C 语言特性——联合体、枚举、位域和 typedef。它们都有一个共同的主题:都是 C 语言在"灵活性"和"安全性"之间选择灵活性的典型案例。联合体让你在内存上变戏法(代价是你得自己记得当前活跃成员是谁),枚举让整数有了名字(代价是它本质还是 int,挡不住非法赋值),位域让你按位编排(代价是可移植性交给编译器),typedef 让复杂类型变得清爽(代价是用过头反而把信息藏起来)。在嵌入式工程里这四个特性是组合拳,看懂它们,你才算拿到了读寄存器头文件的钥匙。
