...
- 99
- 11
description: 掌握 C 语言的文件操作和标准库核心工具，包括文件读写、格式化 I/O、命令行参数处理，搭起 stdio/stdlib 的基础框架
difficulty: beginner
order: 20
platform: host
prerequisites:
...
C 语言的文件操作建立在一套简洁但足够强大的 API 之上——`fopen` 打开、`fread`/`fwrite` 读写、`fclose` 关闭，外加 `printf`/`scanf` 家族做格式化输入输出。这些函数从 1970 年代一直活到今天。但它们也带着那个年代特有的粗糙感——类型不安全、错误处理靠全局变量、格式字符串和参数不匹配时编译器睁一只眼闭一只眼。

> 本章是文件 I/O 与标准库的**入门导览**。C-Journey 后续（stage 5）会有一篇更深入的文件 I/O 文档，专门讲缓存、定位、大文件、跨平台细节等进阶话题；这里先把最常用的 API 和最容易踩的坑理清楚，够你写出第一个能读写文件的程序。
...
文件操作的核心是 `FILE*` 和 `fopen`/`fclose`/`fread`/`fwrite`，格式化 I/O 靠 `printf`/`scanf` 家族，错误处理靠 `errno` + `perror`。标准库提供了数值转换、排序搜索、数学函数、字符分类、断言等基础工具。把这些 API 用熟，你就能写出第一个真正能和磁盘打交道的 C 程序；更深入的缓存、定位、大文件话题留给后续章节。
