...
- 99
- 11
description: 掌握函数指针的声明与使用，理解回调函数模式在事件驱动编程中的应用，从 qsort 这个经典案例看算法与策略的解耦
...
我们在前面的教程里已经系统梳理了指针的各种用法，这篇就专门来啃函数指针这块硬骨头。先从声明和基本用法入手，然后过渡到函数指针数组、回调模式，最后搭一个事件分发系统把所有东西串起来。
