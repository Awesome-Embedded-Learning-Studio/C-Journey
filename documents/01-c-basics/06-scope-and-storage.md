...
C99 之后,`for` 循环的初始化部分也可以声明变量,这个变量的作用域是整个循环(包括循环体和条件判断部分),在循环外不可见。如果你用的是古老的 C89 编译器(一般不太可能了),循环变量必须在循环外声明。
...
static int g_internal_var = 42;  // static 限制了链接性,但作用域仍是文件级
...
int g_max_retry_count = 3;  // 定义,分配内存
const char* g_server_address = "192.168.1.100";
```
...
extern int g_max_retry_count;  // 声明,不分配内存
...
int main(void) {
    printf("Server: %s, Retry: %d\n", g_server_address, g_max_retry_count);
    return 0;
}
```
...
```c
// 头文件里
extern int g_value = 42;  // 千万别这么干!
```

如果给 `extern` 声明同时赋了初值,`extern` 就被忽略了——这变成了一个定义。如果这个头文件被多个 `.c` 文件 `#include`,每个翻译单元都会生成一个 `g_value` 的定义,链接的时候就会收到 `multiple definition` 错误。

> ⚠️ **踩坑预警**
> 头文件里放 `extern int g_value = 42;` 是典型的错误写法——带初始值的 `extern` 等于定义,头文件被多次 include 就会导致链接冲突。记住:头文件里只放声明(不带初始值),定义放 `.c` 文件里。
...
int g_global_var = 10;             // 静态生命周期,数据段
static int g_internal_var = 20;    // 静态生命周期,数据段,内部链接
int g_uninitialized;               // 静态生命周期,BSS 段,自动为 0
...
> 上面这段顺手补了一处原稿的小疏漏:示例里 `heap_var` 是指针,`printf` 必须解引用成 `*heap_var` 才能打印它指向的值,直接传 `heap_var` 打出来的是地址。这是真实编译跑通后修正的,不是脑补。

一个容易忽略的事实是:全局变量的初始化顺序在同一个翻译单元内是确定的(按照定义顺序),但跨翻译单元的初始化顺序是**未定义的**。对于 C 语言来说这通常不是大问题,因为全局变量一般用常量表达式初始化,不会在初始化阶段产生跨文件的依赖。
