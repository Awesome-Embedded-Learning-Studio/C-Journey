...
C 语言里没有真正的"字符串类型"——这是每一个和 C 打交道的开发者都会发出的感叹。在 C 的世界里，字符串就是一块以 `\0` 结尾的 `char` 数组，所有的操作都建立在这个约定之上。这个约定简单到令人感动，也脆弱到令人崩溃——你忘记写那个 `\0`，整个程序的行为就是未定义的；你把一个 100 字节的字符串拷进 50 字节的缓冲区，缓冲区后面的内存就被你踩烂了。

历史上无数的安全漏洞，从早期的 Morris Worm 到近年的各种 CVE，追根溯源都指向同一件事：**缓冲区溢出**。这篇教程我们要做的就是把 C 字符串从里到外拆一遍，搞清楚它的本质、掌握安全操作的手法、认识那些经典的陷阱。
...
```bash
gcc -Wall -Wextra -std=c11 str_copy.c -o str_copy && ./str_copy
```
...
```bash
gcc -Wall -Wextra -std=c11 snprintf_demo.c -o snprintf_demo && ./snprintf_demo
```
...
```bash
# 推荐的开发编译命令
gcc -std=c11 -Wall -Wextra -g -fsanitize=address -fstack-protector-all your_code.c
```
...
C 字符串就是一个以 `\0` 终止的 `char` 数组，没有类型系统的保护，所有安全责任都在程序员身上。`string.h` 提供的函数族是操作字符串的基本工具，不带长度限制的版本（`strcpy`、`strcat`、`sprintf`）是缓冲区溢出的主要来源，应优先使用带 `n` 的版本或者 `snprintf`。`memcpy` 用于不重叠的内存复制，`memmove` 用于可能重叠的情况。编译器选项（`-fstack-protector`、`-D_FORTIFY_SOURCE=2`、`-fsanitize=address`）提供了额外的安全网。理解底层的 C 字符串模型，是在任何 C 项目里写好字符串操作的前提。
