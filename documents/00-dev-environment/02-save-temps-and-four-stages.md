...
> 🟡 状态：待审核（2026-06-30）｜分支：next｜声音锚点件之一，请重点校准语气与方法。
...
> **踩坑预警**：`-save-temps` 会**污染你的源码树**。如果你在项目根目录跑它，这些 `.i`/`.s`/`.o` 会散落得到处都是，甚至可能被 `git add .` 误卷进去。正确做法是**在一个专门的临时目录里跑**（本书示例都放在 `/tmp/` 下），或者用 `-save-temps=obj` 配合 `-o` 指定到构建目录。别图省事在源码树里裸跑 `-save-temps`。

## 坑三：AT&T 还是 Intel 汇编，读反了全章看不懂
...
到这一章，gcc 的黑盒就被我们撬开了。请你确认这几条都吃透了：

- **四阶段四产物**：预处理（`.i`）→ 编译（`.s`）→ 汇编（`.o`）→ 链接（可执行）；对应停靠开关 `-E`/`-S`/`-c`。
- **「编译」严格说只是第二步**；从 `.c` 到可执行整段应叫「翻译/构建」。术语别混。
- **预处理是文本替换**：`#include` 把头文件塞进来（`hello.c` 8 行 → `.i` 几百行）、宏被展开（`GREET` → `"hello from C"`）。这是 C 标准 §5.1.1.2 翻译阶段的体现。
- **`.o` 不能直接跑**：它是 `relocatable`，地址未填实，必须链接成 `executable`。
- **`-save-temps` 方便但污染源码树**：在临时目录里跑，别在项目根裸跑。
- **汇编有两种语法**：gcc 默认 AT&T（`%`/`$`/源在前），`-masm=intel` 切 Intel；操作数顺序相反，读反了全错。
