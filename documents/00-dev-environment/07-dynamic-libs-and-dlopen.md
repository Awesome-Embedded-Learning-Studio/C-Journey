...
> 🟡 状态：待审核（2026-06-30）｜分支：next｜本机 gcc 16.1.1、glibc 2.43、x86-64、全程 `-std=c11` 真跑。
...
第 5 章那个 `printf@GLIBC` 一直留 `U`，现在彻底清楚了——它是动态库符号，按上表最底行的规则，运行期才填。

## 常见坑

- **`dlopen` 的路径别写相对的**：带斜杠的路径按字面找，换 cwd 就 `cannot open shared object file`。用绝对路径、装系统目录、或设 `LD_LIBRARY_PATH`。
- **编 `.so` 永远带 `-fPIC`**：简单代码在现代 gcc 上不加也不报错，但别赌——复杂代码/老工具链/某些架构会直接 `recompile with -fPIC`。
- **`-ldl` 看 libc 版本**：glibc 2.34+ 已并进 libc、非必需；老 glibc/musl 仍必需。移植写法照带 `-ldl`，两头都对。
- **`dlsym` 的 `void*` 转函数指针**：ISO C 不保证（§6.3.2.3），POSIX 保证可行；别拿这个转换去 C 标准里找依据。
- **库升级要守 ABI**：换 `.so` 不用重链程序是动态库的优点，但前提是新库和旧库 ABI 兼容（函数签名、结构体布局没变），否则运行期崩溃——这是动态库灵活性的代价。

## 小结

- **动态库 `.so` = 运行期加载**：链接期只记「我要哪个库的哪个符号」，代码不进可执行，启动时由动态链接器填地址（`printf@GLIBC` 留 `U` 的真相）。
- **`-fPIC -shared` 编 `.so`**：位置无关代码是 `.so` 的硬要求（尽管简单代码在这台 gcc 上不加也编得过，规矩仍要带）。
- **`dlopen/dlsym/dlclose`**：运行期加载、用字符串名取符号、用完关掉；`dlsym` 返回 `void*` 转函数指针靠 POSIX 保证，非 ISO C。
- **真坑**：`dlopen` 相对路径（换 cwd 就崩）；`-ldl` 在新 glibc 非必需、老的必需。
