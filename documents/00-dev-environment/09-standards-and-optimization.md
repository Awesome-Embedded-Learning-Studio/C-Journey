...
上一章末尾我们留了个扣子：这一章要把 `-std` 各档、`-O0..-O3` 优化级别、`-g` 调试信息一起讲透，顺带解释为什么第 8 章好几个警告旗标（尤其 `-Wuninitialized`）的行为会和 `-O` 级别绑在一起。这三个旋钮几乎在每个 Makefile、每个 CMakeLists 里都会出现，但我打赌很多人是照着别人的抄来的，并没有真懂——有人分不清 `-std=c11` 和 `-std=gnu11` 到底差在哪，有人以为 `-O0` 就是「完全不优化所以最安全」，有人不知道 `-g` 和 `-O` 互相牵扯。
...
int main(void) {
    /* GNU statement expression：整块的值是最后一条表达式的值。标准 C 没有这玩意。 */
    int x = ({
        int tmp = 3;
        tmp + 4;
    });
