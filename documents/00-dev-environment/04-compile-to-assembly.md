...
> **踩坑预警**：别以为 `const` 修饰的全局就一定进 `.rodata`。`const` 是写给编译器看的「语义约束」，**一个 `const` 变量最终进哪段，取决于它有没有被初始化、有没有被取地址、链接器怎么摆**——很多 `const` 全局实际落在 `.data` 或 `.rodata` 的子段里。判断它到底在哪，靠 `readelf -S` / `objdump`，不靠猜。这章我们只摆事实，`const` 的精细语义留到第 2 编讲限定符时再展开。
...
> **踩坑预警**：`-O2` 不只是「让程序变快」，它会**改变可观察行为的边界**——尤其是碰到未定义行为（UB）的时候。一段依赖 UB 的代码（比如有符号溢出、越界），在 `-O0` 下可能「看起来正常」，一上 `-O2` 优化器「假定它不会 UB」就直接把你的溢出检查整段删掉，输出变得完全错误。这是后面为什么要上 sanitizer 的根本原因，第 10 章会专讲。
...
> **踩坑预警**：想让编译器在这个坑上提醒你，得手动加 `-Wwrite-strings`（它不在 `-Wall`/`-Wextra` 里）——这个开关会把字符串字面量当成 `const char[]`，于是 `char *s = "hello"` 立刻报警。本课程后续示例里，凡是不该被改的字符串，我们都老老实实写 `const char *s`，从源头堵住这个雷。

## 小结

到这，编译阶段产出的 `.s` 你就能读个大概了。请把这几条焊死：

- **程序是分段的**：`.text`（代码，可执行）/`.rodata`（只读）/`.data`（已初始化可写）/`.bss`（零初始化、不占文件空间）。变量落哪段，看「初始化没」「只读不」，用 `size`、`readelf -S`、`objdump` 验，不靠猜。
- **段与寄存器是 ABI，不是 C 标准**：ISO C 对 `.text/.data`、对 `rdi` 只字未提；这是 ELF + System V AMD64 ABI 的现实，换平台会变。
- **调用约定**：x86-64 SysV 下整型参数前 6 个走 `rdi/rsi/rdx/rcx/r8/r9`，第 7 个起走栈；栈帧里 `rbp+8` 是返回地址。
- **`-O0` vs `-O2`**：`-O0` 局部变量全在栈上（好调试），`-O2` 会按 as-if 规则（§5.1.2.3）吃掉不影响可观察行为的变量（gdb 里 `<optimized out>`）。**调试用 `-O0 -g`，基准才用 `-O2`。**
- **字符串字面量不可改**（§6.4.5）：写它就是 UB，在 `.rodata` 上表现为 `SIGSEGV`；C 默认不警告，要 `-Wwrite-strings` 才提醒你。
