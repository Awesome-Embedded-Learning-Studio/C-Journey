...
> 🟡 状态：待审核（2026-06-30）｜分支：next｜本机 gcc 16.1.1、ld、x86-64、全程 `-std=c11` 真跑。
...
- **链接器干两件事**：符号解析（把 `U` 配上定义）+ 重定位（回填地址）。
- **`undefined reference`**：用了某符号但没给它的定义来源（`.o` 或库）。报错会指明在哪个 `.o`、哪个函数引用的。
- **`multiple definition`**：同名强定义撞车（语言层违反 §6.9¶5，是 UB；实践中链接器硬拦）。拼装旧代码高发，先全局查重名。
- **静态库 `libxxx.a`** = `ar rcs` 打包的一堆 `.o`；链接用 `-lxxx -L<dir>`。`ar rcs` 的 `s` 保证有符号索引。
- **库顺序陷阱**：链接器从左到右单趟扫描，**对象在前、库在后、被依赖的放右边**。报错和"漏链"一样，排查时先看顺序再改代码。
