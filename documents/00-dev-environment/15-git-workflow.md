...
上面演示的都是本地仓库。真实协作里，你还有一个**远程仓库**（GitHub/GitLab 上的那个仓库），本地和远程之间靠 `git push`（本地→远程）和 `git pull`（远程→本地）同步。这部分本项目有几条**必须遵守的硬约定**（来自 [AGENTS.md](../../AGENTS.md)），你在这个仓库里干活前一定要知道：
...
- 本项目 [AGENTS.md](../../AGENTS.md)（`next` 分支、不 push、commit 不带 Co-Authored-By 的硬约定）
