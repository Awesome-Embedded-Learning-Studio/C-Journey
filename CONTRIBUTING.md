...
C-Journey 本身就脱胎于维护者的 C 学习笔记,所以特别欢迎你也把自己的笔记整理成章投上来——无论改一个错别字、补一个示例,还是一整章,都热烈欢迎,小贡献一样算数。

## 仓库地图

| 路径 | 作用 |
|---|---|
| `documents/` | 各阶段知识点文档(主线六阶段 84 章 + 路线图/更新日志;嵌入式/capstone 尚未动笔) |
| `examples/` | 配套可编译示例(按阶段) |
| `projects/` | 完整项目(clib-utilities 已整改进 CI 硬门;os-from-scratch 等为参考实现,改动前开 Issue) |
| [documents/roadmap.md](./documents/roadmap.md) | 八阶段学习路线图(0-5 已上线) |
...
- **就事论事**:对代码 / 内容的批评对事不对人。
- **接纳反馈**:被指出错误时理性响应,纠正就好。
- **乐于互助**:能回答新手问题就回答,营造「折腾工程师同行者」的氛围。

骚扰、人身攻击、仇恨言论、持续性打扰,或其他让他人感到不适的行为不可接受,维护者有权关闭、移除此类言论或当事人。遇到违反准则的行为,请通过 GitHub 联系仓库维护者(私下报告)。
